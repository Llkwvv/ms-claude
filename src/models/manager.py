...
class ModelManager:
    ...
    def reload_config(self) -> None:
        """重新加载模型管理器配置（支持热重载）。"""
        self._priority_config = self.config.get("model_priority", [])
        self._cache_ttl = self.config.get("modelscope.cache_ttl", 86400)
        self.logger.info("ModelManager config reloaded")
    ...
    def _resolve_fetch_script_path(self) -> Optional[Path]:
        ...
        for candidate in [
            repo_root / "scripts" / "fetch_text_generation_models.py",
            repo_root / "fetch_text_generation_models.py",
        ]:
            if candidate.exists():
                return candidate
        return None
